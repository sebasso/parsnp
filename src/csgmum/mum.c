...
inline void print_state(UM *Pair, int size)
{
    int i;
    printf("    ");
    for (i = 0; i < size; i++) {
...
inline void Intersect_UM(CSG *csg, UM *Master, UM *Pair, int size, ulong *SP)
{
    int k = 1;
    int MMP;
...
inline void Find_UM(CSG *csg, const char *s,ulong *SP, UM *Pair)
{
    char *pendent = (char *)s;
    int  lprefix  = 0;
    CSG_Node *state;
